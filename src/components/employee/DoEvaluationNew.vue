<template>
    <div id="doEvaluationNew" class="container ml-6">           
    ...
    <div v-if="showQ">
        ...
        <form v-if="item.type === 3" class="form-group ml-3 justify-content-start">
            <div class="card shadow-lg mb-5">
            <div class="card-body">
            ...
            <span v-for="(optionItem, index1) in item.option" v-bind:key="index1 + 1">
        ...
        <form v-if="item.type === 2" class="mt-3 ml-3">
            ...
            <div class="form-group ml-3 justify-content-start">                
                ...
                <div class="row ml-2"><p class="col-1.5">題目內容：</p><p class="col-10">{{item.content}}</p></div>   
                    <div id="likertScale" class="col ml-0 mt-1">
                        <div class="form-check form-check-inline mr-4">
                            <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value=1 v-model="item.response">
                            <label class="form-check-label ml-2" for="inlineRadio1">1</label>
                        </div>
        ...
        <div class="row mb-5 d-none d-sm-block" style="margin-left: 40%; margin-top: 6%">
            <div class="form-inline">
                ...
                <button type="button" class="btn btn-primary btn-lg ml-5" @click="sendout">送出</button>                    
            </div>
        </div>
...
export default {
    ...
    created() {
        ...
        this.tmpSave = this.$route.query.tmpSave;
        this.tmpSaveData = this.$route.query.tmpSaveData;
            console.log(this.tmpSave);
        ...
        let func = async () => {
            try{
                for(let i = 0; i < this.questionsList.length; i++) {
                    if (this.questionsList[i] == '5c480c3ec2b976371c7f03f1'){
                        continue;
                    }
    ...
    methods: {        
        ...
        restoreTmpData: function() {
            console.log(this.questionsFiltered);    
            
            for (let i = 0; i < this.questionsFiltered.length; i++){                
                for (let j = 0; j < this.questionsFiltered[i].length; j++) {
                    for (let k = 0; k < this.tmpSaveData.topic.length; k++) {
                        if (this.questionsFiltered[i][j]._id === this.tmpSaveData.topic[k].topic_id._id) {
                            switch (this.questionsFiltered[i][j].type) {
                                case 1:
                                case 3:
                                    this.questionsFiltered[i][j].response = this.tmpSaveData.topic[k].topic_res;
                                    break;
                                case 2:
                                    this.questionsFiltered[i][j].response = this.tmpSaveData.topic[k].topic_score;
                                    break;
                            }
                        }
                    }
            ...
            this.colleageSuggestion = this.tmpSaveData.suggest;
            console.log(this.questionsFiltered);    
